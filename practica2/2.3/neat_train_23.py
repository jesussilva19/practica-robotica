...
def eval_genome(genome, config):
    ...
    env = RoboboNEATAvoidEnv23(max_steps=200)  # igual que 2.2
    obs, _ = env.reset()
    total = 0.0
    steps = 0
    try:
...
def run(config_file, generations=30):
    ...
    print("\nIniciando evolución con NEAT (2.3)...")
...
if __name__ == "__main__":
    # MISMA config que 2.1/2.2 (5 inputs, 6 outputs, sin threshold que corte pronto)
    config_path = "practica2/2.3/config-feedforwardmod"  # ajusta la ruta a la tuya
    if not os.path.exists(config_path):
        raise FileNotFoundError(f"No se encuentra config: {config_path}")
    run(config_path, generations=30)
